      ...
      queue.textChannel
        .send({
          embeds: [
            new EmbedBuilder()
              .setColor(client.config.embed.color)
              .setDescription(`⚠ หมดคิวแล้วครับ ไม่มีเพลงให้เล่นอีกต่อไป`),
          ],
        })
        .then((msg) => {
          setTimeout(() => {
      ...
      message.channel
        .send({
          embeds: [
            new EmbedBuilder()
              .setColor(client.config.embed.color)
              .setDescription(`ฉันไม่สามารถค้นหา \`${quary}\``),
          ],
        })
        .then((msg) => {
          setTimeout(() => {
          ...
          case "autoplay":
            {
              if (!channel) {
                ...
                return send(
                  interaction,
                  ` ${client.config.emoji.ERROR} คุณต้องเข้าร่วม ____ ช่องเสียง `
                );
              } else if (!queue) {
                return send(
                  interaction,
                ...
                return send(
                  interaction,
                  `${client.config.emoji.SUCCESS} คุณไม่ใช่ดีเจและไม่ใช่ผู้ขอเพลงด้วย..`
                );
              } else if (!queue.autoplay) {
                queue.toggleAutoplay();
                return send(
                  interaction,
                  ` ${client.config.emoji.SUCCESS} เปิดใช้งานการเล่นอัตโนมัติแล้วครับ `
                );
              } else {
                queue.toggleAutoplay();
                return send(
                  interaction,
                  ` ${client.config.emoji.SUCCESS} ปิดการเล่นอัตโนมัติแล้วครับ `
                );
              }
            }
            break;
          case "skip":
            {
              if (!channel) {
                return send(
                  interaction,
                  ` ${client.config.emoji.ERROR} คุณต้องเข้าร่วมช่องเสียงนี้ครับ`
                );
              } else if (
                interaction.guild.members.me.voice.channel &&
                !interaction.guild.members.me.voice.channel.equals(channel)
                ...
                return send(
                  interaction,
                  `${client.config.emoji.SUCCESS}  ข้ามเพลงนี้แล้วครับ `
                );
              }
            }
            break;
          case "stop":
            {
              if (!channel) {
                ...
                return send(
                  interaction,
                  ` ${client.config.emoji.SUCCESS} เพลงหยุดแล้ว ช่องซ้าย!!.`
                );
              }
            }
            break;
          case "pauseresume":
            {
              if (!channel) {
                ...
                return send(
                  interaction,
                  ` ${client.config.emoji.SUCCESS} ต่อคิว!! `
                );
              } else if (!queue.paused) {
                await queue.pause();
                return send(
                  interaction,
                  ` ${client.config.emoji.SUCCESS} คิวเพลงหมดแล้วครับ  `
                );
              }
            }
            break;
